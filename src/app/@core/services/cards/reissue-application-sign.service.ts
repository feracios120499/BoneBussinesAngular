...
export class ReissueApplicationSignService extends BaseService {
  ...
  getBuffer(applicationsId: number, clientId: string): Observable<SignBuffer> {
    return this.getBuffers([applicationsId], clientId).pipe(
  ...
  getBuffers(
    ...
    clientId: string
  ): Observable<SignBuffer[]> {
    return this.http.post<SignBuffer[]>(
      `api/v1/corpcards/reissue/buffer/${clientId}`,
      applicationIds
    );
  }
  ...
  addSignature(
    sign: SignRequest,
    clientId: string
  ): Observable<SignSaveResponse> {
    return this.addSignatures([sign], clientId).pipe(
      map((result) => result[0])
    );
  }
  ...
  addSignatures(
    ...
    return this.http.post<SignSaveResponse[]>(
      `api/v1/corpcards/reissue/sign/${clientId}`,
      signatures
    );
  }
}
