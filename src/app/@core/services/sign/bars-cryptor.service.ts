...
export class BarsCryptorService implements BaseSignService {
    ...
    init(token: CryptorToken): Observable<CryptorResponse> {
        return this.http.post<CryptorResponse>(`${CRYPTOR_URL}/init`, this.toPascalCase(token)).pipe(
            map(response => this.mapResponse(response, response))
        );
    }

    getKeys(token: CryptorToken): Observable<CryptorKey[]> {
        return this.http.post<CryptorKeysResponse>(`${CRYPTOR_URL}/keys`,
            this.toPascalCase(token)
        ).pipe(
            map(response => this.mapResponse(response, response.keys))
        );
    }
    ...
    private sign(data: any): Observable<string> {
        return this.http.post<CryptorSign>(`${CRYPTOR_URL}/sign`, this.toPascalCase(data)).pipe(
            map((response) => this.mapResponse(response, response.sign)));
    }
