...
export namespace CardsSelectors {
    ...
    export const openCardAccounts = createSelector(
        cardsStore,
        state => state.openAccounts
    );

    export const uiCardAccountsSelector = createSelector(
        cardsStore,
        openCardAccounts,
        (state, openAccounts) => state.cardAccounts.map(p => ({ ...p, isOpen: openAccounts.includes(p.id) }))
    )
}