...
export class PayFormsEffects {
            ...
            switchMap(payload =>
                this.paymentsService.createWithinCountryPayment(
                    ...
                    payload.clientId).pipe(
                        switchMap(response => this.paymentsService.getPayment(response.id, payload.clientId).pipe(
    ...
    createMyAccounts$ = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(action => clientIdWithData(this.store, action.payload)),
            switchMap(payload =>
                this.paymentsService.createMyAccountsPayment(
                    payload.data,
                    payload.clientId).pipe(
                        switchMap(response => this.paymentsService.getPayment(response.id, payload.clientId).pipe(
    ...
    createSignAndToBank$ = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(_ => this.actions$.pipe(ofType(
                PayFormsActions.createWithinCountrySuccess,
                PayFormsActions.createMyAccountsSuccess
            )).pipe(take(1))),
