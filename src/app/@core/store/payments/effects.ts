...
export class PayEffects {
  ...
  signPayment$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((action) => clientIdWithData(this.store, action.payload)),
      switchMap((payload) =>
        this.paymentsSignService.getBuffer(payload.data, payload.clientId).pipe(
  ...
  onSign$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((action) => clientIdWithData(this.store, action.payload)),
      switchMap((payload) =>
        this.paymentsService.onSign(payload.data, payload.clientId).pipe(
          map((response) =>
            response.isSuccess
  ...
  toBank$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((action) => clientIdWithData(this.store, action.payload)),
      switchMap((payload) =>
        this.paymentsService.toBank(payload.data, payload.clientId).pipe(
          map((response) =>
            response.isSuccess
      ...
      map(() => {
        const config: CorrespondentsModalConfig = {
          callback: (result: CorrespondentsModalResult) => {
            this.store.dispatch(
              SharedActions.setCreatePartialPayment({
                payment: { recipient: result as PaymentAccount },
              })
            );
          },
        };
