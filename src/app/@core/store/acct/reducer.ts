...
export const acctReducer = createReducer(
    ...
    on(
        ...
        (state) => ({ ...state, loadings: [...state.loadings, AcctLoadings.list] })
    ),
    on(
        AcctActions.setAccounts,
        (state) => ({ ...state, loadings: state.loadings.filter(p => p !== AcctLoadings.list) })
    ),
    on(
        AcctDetailsActions.loadAccount,
        (state) => ({ ...state, loadings: [...state.loadings, AcctLoadings.details] })
    ),
    on(
        AcctDetailsActions.loadAccountSuccess,
        AcctDetailsActions.loadAccountFailure,
        (state) => ({ ...state, loadings: state.loadings.filter((p => p !== AcctLoadings.details)) })
    ),
    on(
        AcctDetailsActions.loadTurnoversRequest,
        (state) => ({ ...state, loadings: [...state.loadings, AcctLoadings.turnovers] })
    ),
    on(
        AcctDetailsActions.loadTurnoversSuccess,
        AcctDetailsActions.loadTurnoversFailure,
        (state) => ({ ...state, loadings: state.loadings.filter(p => p !== AcctLoadings.turnovers) })
    ),
    on(
        AcctDetailsActions.loadTransactionsRequest,
        (state) => ({ ...state, loadings: [...state.loadings, AcctLoadings.transactions] })
    ),
    on(
        ...
        (state) =>
            ({ ...state, loadings: state.loadings.filter((value, index) => index !== state.loadings.indexOf(AcctLoadings.transactions)) })
    ),
    on(
        AcctDetailsActions.loadTransactionDetailRequest,
        (state) => ({ ...state, loadings: [...state.loadings, AcctLoadings.transaction] })
    ),
    on(
