...
export class AcctDetailsEffects implements OnRunEffects {
  ...
  setAccountName$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(
        ([action, formControl]) =>
          new SetValueAction(formControl.controls.name.id, action.name)
      )
    )
  );
  ...
  loadAccountSuccess$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map((action) =>
        AcctDetailsActions.setCurrentAccount({ account: action.payload })
      )
    )
  );
    ...
    this.actions$.pipe(
      ...
      switchMap(([payload, routeParams]) =>
        this.accountsService
          ...
          .pipe(
            withLatestFrom(
              this.store.select(AcctDetailsSelectors.currentAccount)
            ),
            ...
            catchError((error) =>
              of(AcctDetailsActions.updateAccountFailure(error.error.Message))
            )
          )
      )
    )
      ...
      map(() =>
        NotifyActions.successNotification({
          message: this.translateService.instant(
            'componets.acct.updateAccountSuccess'
          ),
        })
      )
    )
  ...
  updateRangeTransactions$ = createEffect(() =>
    this.actions$.pipe(
      ...
      filter(
        ([formControl, form]: [
          SetValueAction<Boxed<DateRangeString>>,
          FormGroupState<AcctTransactionsFilter>
    ...
    this.actions$.pipe(
      ...
      switchMap((payload) =>
        this.accountsService
          ...
          .pipe(
            map((turnovers) =>
              AcctDetailsActions.loadTurnoversSuccess(turnovers)
            ),
            catchError((error) => {
              return of(
  ...
  closeTurnover$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(([action]) =>
        AcctDetailsActions.loadTransactionsCancel({ id: action.id })
      )
    )
  );
      ...
      switchMap((action) =>
        clientIdWithData(this.store, action.payload).pipe(
          withLatestFrom(
            notNullAndUndefined(
              this.store,
              AcctDetailsSelectors.turnover(action.payload)
            )
          )
        )
      ),
  ...
  loadTransactionsFailure$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map((action) =>
        AcctDetailsActions.closeTurnovers({ id: action.payload.id })
      )
    )
  );
    ...
    this.actions$.pipe(
      ...
      switchMap((payload) =>
        this.accountsService
          ...
          .pipe(
            map((transaction) =>
              AcctDetailsActions.loadTransactionDetailSuccess(transaction)
            ),
            catchError((error) =>
              of(
    ...
    this.actions$.pipe(
      ...
      switchMap(([action, account]) => {
        ...
        const payment: Partial<PaymentModal> = {
          ...
          sender:
            transacion.credit > transacion.debit
    ...
    this.actions$.pipe(
      ...
      map(([action, account]) => {
        ...
        const payment: PaymentModal = {
          ...
          currencyCode:
            transaction.sender.accCurrencyCode ||
            transaction.recipient.accCurrencyCode,
          sender: {
  ...
  showExportModal$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(([, account, range]) =>
        AcctActions.openExportModal({ account, range })
      )
    )
  );
  ngrxOnRunEffects(
    ...
    return this.actions$.pipe(
      ...
      exhaustMap(() =>
        resolvedEffects$.pipe(
          takeUntil(
            this.actions$.pipe(ofType(AcctDetailsActions.destroyDetails))
          )
        )
      )
    );
