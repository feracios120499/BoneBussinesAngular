    ...
    this.action$.pipe(
      ...
      switchMap((action) =>
        this.action$.pipe(
          ...
          switchMap((otpAction) =>
            this.authService
              .loginWithOtp(action.payload, otpAction.payload)
              .pipe(
                map((token) => AuthActions.loginWithOtpSuccess(token)),
                catchError((error) =>
                  of(
                    AuthActions.loginWithOtpFailure(
    ...
    this.action$.pipe(
      ...
      switchMap((action) =>
        this.authService.loginWithData(action.payload).pipe(
          ...
          catchError((error) =>
            of(AuthActions.tokenFailure(error.error.errorDescription))
          )
        )
      )
    )
    ...
    this.action$.pipe(
      ...
      switchMap((_) =>
        this.cryptorService.getTokens().pipe(
          ...
          catchError((error) =>
            of(AuthActions.loadCryptorTokensFailure(error.message))
          )
        )
      )
    )
        ...
        this.cryptorService.init(action.payload).pipe(
          switchMap((_) =>
            this.cryptorService.getKeys(action.payload).pipe(
              ...
              catchError((error) =>
                of(AuthActions.loadCryptorKeysFailure(error.message))
              )
            )
          ),
          catchError((error) =>
      ...
      switchMap((action) =>
        this.cryptorService
          ...
          .pipe(
            switchMap((sign) =>
              this.authService
                ...
                .pipe(
                  ...
                  catchError((error) =>
                    of(
                      AuthActions.loginCryptorFailure(
                        error.error.errorDescription
                      )
                    )
                  )
            ...
            catchError((error) =>
              of(AuthActions.loginCryptorFailure(error.message))
            )
          )
      )
    )
