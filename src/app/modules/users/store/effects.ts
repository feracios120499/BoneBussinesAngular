...
import { UsersService } from '@modules/users/services/users-service/users.service';
...
export class UsersEffects {
  ...
  setFoundUser$ = createEffect(() =>
    this.actions$.pipe(
      ...
      tap(({ user }) => this.store.dispatch(UsersActions.setFoundUser({ user }))),
    ...
    this.actions$.pipe(
      ...
      switchMap(() => [
        ...
        NotifyActions.successNotification({
          message: this.translateService.instant('components.admin.newUserIsAdded'),
        }),
      ])
    )
  );
        ...
        this.usersService.updateUserRoles(clientId, userId, { roles }).pipe(
          ...
          catchError((error) =>
            of(
              ...
              NotifyActions.serverErrorNotification({
                error,
                message: this.translateService.instant('errors.updateUserRoles'),
              })
            )
          )
        )
  ...
  updateUserLockStateRequest$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ clientId, data: { userId, isLock } }) =>
        this.usersService.updateUserLockState(clientId, userId, { isLock }).pipe(
          ...
          catchError((error: ServerError) =>
            of(
              ...
              NotifyActions.serverErrorNotification({
                error,
                message: this.translateService.instant('errors.updateUserLockState'),
              })
            )
          )
        )
  ...
  deleteUserSuccess$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(() => [
        UsersActions.loadUsersRequest(),
        NotifyActions.successNotification({
          message: this.translateService.instant('components.admin.userDeleted'),
        }),
      ])
    )
  );
