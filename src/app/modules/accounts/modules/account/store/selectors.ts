...
export namespace AcctDetailsSelectors {
  ...
  export const currentAccount = createSelector(
    ...
    (acctState, detailState, routeParams) =>
      detailState?.account || acctState.accounts?.find((p) => p.id === routeParams.accountId)
  );

  export const filterTransactions = createSelector(detailStore, (state) => state?.transactionsFilterForm);

  export const transactionsRange = createSelector(filterTransactions, (state) => ({
    start: dayjs(state?.value.range.value.start),
    end: dayjs(state?.value.range.value.end),
  }));

  export const transactionsRangeString = createSelector(transactionsRange, (range) => ({
    start: range.start.format('DD.MM.YYYY'),
    end: range.end.format('DD.MM.YYYY'),
  }));
  ...
  export const openTurnovers = createSelector(detailStore, (store) => store.openTurnovers);

  export const loadTurnovers = createSelector(detailStore, (store) => store.loadTurnovers);
