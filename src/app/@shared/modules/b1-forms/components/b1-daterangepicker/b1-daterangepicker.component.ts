import { ChangeDetectorRef, Component, ElementRef, EventEmitter, forwardRef, Input, OnInit, Output, TemplateRef, ViewChild, ViewEncapsulation } from '@angular/core';
...
export class DaterangepickerComponent implements OnInit {
  ...
  renderCalendar(side: SideEnum) { // side enum
    ...
    for (let i = 0, col = 0, row = 0; i < 42; i++, col++, curDate = dayjs(curDate).add(24, 'hour')) {
      ...
      const minDateLocal = this.getMinDate();
      if (minDateLocal && calendar[row][col].format('YYYY-MM-DD') === minDateLocal.format('YYYY-MM-DD') &&
        calendar[row][col].isBefore(this.getMinDate()) && side === 'left') {
        calendar[row][col] = minDateLocal.clone();
      }

      const maxDateLocal = this.getMaxDate();
      if (maxDateLocal && calendar[row][col].format('YYYY-MM-DD') === maxDateLocal.format('YYYY-MM-DD') &&
    ...
    for (let row = 0; row < 6; row++) {
      ...
      for (let col = 0; col < 7; col++) {
        ...
        if (
          ...
          (
            calendar[row][col] > this.startDate && (this.endDate) &&
            (
              calendar[row][col] < this.endDate || (calendar[row][col] < (this.nowHoveredDate as any) && this.pickingDate)
            )
          ) &&
          (
