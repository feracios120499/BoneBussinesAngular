...
export class IbanDirective implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (this.iban.length < this.ibanService.IbanLength) {
      this.elementRef.nativeElement.innerHTML = `<span>${this.iban}</span>`;
      return;
    }
    if (changes.iban.currentValue !== changes.iban.previousValue && this.ibanService.isIban(this.iban)) {
      ...
      let zeros = '';
      while (
        countryCode.length + mfo.length + checkNumbers.length + zeros.length + accountNumber.length !==
