...
export class MoreAutoDirectionDirective implements OnInit, OnDestroy {
  @Input() parentSelector = '.b1-page-data';
  openSubscription?: Subscription;

  constructor(
    @Self() @Inject(NgbDropdown) private readonly dropdown: NgbDropdown,
    private el: ElementRef
  ) {}

  ngOnInit(): void {
    if (
      this.el.nativeElement.getAttribute('mobile-more') &&
      window.screen.width <= environment.mobileWidth
    ) {
      return;
    }
    ...
    this.openSubscription = this.dropdown.openChange
